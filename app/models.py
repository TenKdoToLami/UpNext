            ...
            data.update(
                {
                    ...
                    "completedAt": (
                        self.user_data.completed_at.isoformat()
                        if self.user_data.completed_at
                        else None
                    ),
...
class MediaCover(db.Model):
    ...
    cover_image: Mapped[Optional[bytes]] = deferred(
...
class MediaMetadata(db.Model):
    ...
    item: Mapped["MediaItem"] = relationship(
        "MediaItem", back_populates="metadata_info"
    )
...
class MediaRelease(db.Model):
    ...
    item: Mapped[Optional["MediaItem"]] = relationship(
        "MediaItem", back_populates="releases"
    )
