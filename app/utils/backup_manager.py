...
def should_backup(db_name: str, frequency_days: int = 7) -> bool:
    ...
    if should_run:
        logger.info(
            f"Backup due for {db_name}: {days_since_backup} days since last backup"
        )
    else:
        logger.debug(
            f"Backup not due for {db_name}: {days_since_backup}/{frequency_days} days"
        )
...
def run_auto_backup(
    db_name: str, settings: Optional[Dict[str, Any]] = None
