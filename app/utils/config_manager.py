...
def save_window_geometry(window: Any) -> None:
    ...
    save_config({"window": {"width": w, "height": h, "x": x, "y": y}})
    logger.info(f"Window geometry saved successfully: ({x}, {y}) {w}x{h}")


def ensure_window_on_screen(
    ...
    if x <= -16000 or y <= -16000:
        logger.warning(
            f"Rejecting minimized sentinel coordinates ({x}, {y}), using OS default"
        )
        return None, None
    ...
    try:
        ...
        for monitor in monitors:
            ...
            if (
                monitor.x <= window_center_x < monitor.x + monitor.width
                and monitor.y <= window_center_y < monitor.y + monitor.height
            ):
                # Window is visible on this monitor
                # Clamp to ensure at least 100px of window is visible
                clamped_x = max(
                    monitor.x - width + 100, min(x, monitor.x + monitor.width - 100)
                )
                clamped_y = max(monitor.y, min(y, monitor.y + monitor.height - 100))

                if clamped_x != x or clamped_y != y:
                    logger.info(
                        f"Clamped window position from ({x}, {y}) to ({clamped_x}, {clamped_y})"
                    )
