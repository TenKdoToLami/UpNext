...
def parse_export_params():
    ...
    return {
        ...
        "filter_types": [
            t for t in request.args.get("filterTypes", "").split(",") if t
        ],
        "filter_statuses": [
            s for s in request.args.get("filterStatuses", "").split(",") if s
        ],
        "filter_ratings": [
            int(r)
            for r in request.args.get("filterRatings", "").split(",")
            if r.isdigit()
        ],
...
def filter_items(items, params):
    ...
    for item in items:
        ...
        if (
            ...
            and item.get("status") not in params["filter_statuses"]
        ):
            continue
        # Rating filter
...
def export_json(items, fields, images):
    ...
    export_items = (
        [filter_item_fields(item, fields) for item in processed_items]
        if fields
    ...
    for item in items:
        ...
        if new_item.get("id") in images:
            ...
            if media_item and media_item.cover and media_item.cover.cover_image:
                ...
                if base64_encode:
                    b64_str = base64.b64encode(media_item.cover.cover_image).decode(
                        "utf-8"
                    )
...
def export_html(items, fields, format_param, images):
    ...
    return send_file(
        ...
        mimetype="text/html",
        as_attachment=True,
        download_name=generate_zip_filename(f"upnext_{format_param}").replace(
            ".zip", ".html"
        ),
    )
...
def export_data():
    ...
    try:
        ...
        if format_param == "json_raw":
            ...
            processed_items = _process_items_for_export(
                items, images, base64_encode=False
            )
            export_items = (
                [filter_item_fields(item, fields) for item in processed_items]
                if fields
            ...
            for root, dirs, files in os.walk(target_root):
                ...
                for file in files:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Could not add file to backup: {rel_path} - {e}"
                        )

            zf.writestr("BACKUP_README.md", _generate_backup_readme())

        return create_zip_response(
            generate_zip_filename("upnext_full_backup"), memory_file
        )
