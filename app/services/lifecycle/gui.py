...
class JsApi:
    ...
    def notify_update(self) -> str:
        ...
        if self.window_ref[0]:
            try:
                self.window_ref[0].evaluate_js("if(window.loadItems) window.loadItems();")
            except Exception as e:
...
def setup_window_closing_handler(window_ref: list, is_dormant: list, tray_available: list, tray_icon: list, target_url: str):
