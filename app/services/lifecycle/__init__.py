...
def run_application_stack(
    ...
    if minimized and not tray_available[0]:
        logger.warning(
            "Cannot start minimized because System Tray is unavailable. Starting normally."
        )
        minimized = False
    ...
    if initial_width < MIN_WIDTH or initial_height < MIN_HEIGHT:
        logger.warning(
            f"Saved window size ({initial_width}x{initial_height}) below minimum, resetting to defaults"
        )
        ...
        try:
            ...
            if os.path.exists(tray_icon_path):
                ...
                action_open.triggered.connect(
                    lambda: run_threaded(actions["on_restore_native"], None, None)
                )

                action_browser = QAction("Open in Browser", menu)
                action_browser.triggered.connect(
                    lambda: run_threaded(actions["on_open_browser"], None, None)
                )

                action_add = QAction("Add", menu)
                action_add.triggered.connect(
                    lambda: run_threaded(actions["on_quick_add"], None, None)
                )

                action_switch = QAction("Switch Library", menu)
                action_switch.triggered.connect(
                    lambda: run_threaded(actions["on_switch_db"], None, None)
                )
            ...
            logger.error(
                f"Failed to setup Linux tray (Manual Mode): {e}", exc_info=True
            )
