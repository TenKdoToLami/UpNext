...
class ExternalAPIService:
    ...
    def search(
        self, query: str, media_type: str, source: Optional[str] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        if source:
            if source == "tmdb":
                ...
                if tmdb_type not in ("Movie", "Series"):
                    return []
                ...
                if media_type not in ("Anime", "Manga", "Movie", "Series"):
                    return []
                search_type = (
                    "Anime" if media_type in ("Movie", "Series") else media_type
                )
        ...
        if media_type == "Anime":
            ...
            if priority_source == "anilist":
                return self.anilist.search(query, "Anime")
            ...
            elif priority_source == "tmdb" and self.tmdb.api_key:
                return self.tmdb.search(query, media_type)
            ...
            if priority_source == "googlebooks":
                return self.googlebooks.search(query, media_type)
            elif priority_source == "comicvine":
                return self.comicvine.search(query, media_type)
            return self.openlibrary.search(query)
    ...
    def get_details(
        self, external_id: str, media_type: str, source: str
    ...
    def update_keys(self, **keys):
        ...
        if "tmdb" in keys:
            self.tmdb.api_key = keys["tmdb"]
        if "googlebooks" in keys:
            self.googlebooks.api_key = keys["googlebooks"]
        if "comicvine" in keys:
            self.comicvine.api_key = keys["comicvine"]
