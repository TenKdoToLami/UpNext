...
class AniListClient(BaseAPIClient):
    ...
    def search(
        ...
        variables = {"search": query, "type": anilist_type, "page": 1, "perPage": 10}
        ...
        for item in data["Page"].get("media", []):
            ...
            studios = [
                s["name"]
                for s in studios_data.get("nodes") or []
                if s and s.get("name")
            ]
            ...
            staff = [
                s["name"]["full"]
                for s in staff_data.get("nodes") or []
                if s and s.get("name") and s["name"].get("full")
            ]
    ...
    def get_details(self, external_id: str) -> Optional[Dict[str, Any]]:
        ...
        studios_data = item.get("studios") or {}
        studios = [
            s["name"] for s in studios_data.get("nodes") or [] if s and s.get("name")
        ]
        ...
        tags = sorted(
            item.get("tags", []), key=lambda t: t.get("rank", 0), reverse=True
        )
        ...
        return {
            ...
            "cover_url": item.get("coverImage", {}).get("extraLarge")
            or item.get("coverImage", {}).get("large"),
