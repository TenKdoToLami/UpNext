    ...
    def add_item(self, data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            new_item = MediaItem(
                ...
                release_date=(
                    datetime.fromisoformat(data["releaseDate"]).date()
        ...
        try:
            ...
            if "rating" in data:
                item.user_data.rating = (
                    data["rating"] if data["rating"] is not None else 0
                )
            item.user_data.progress = data.get("progress", item.user_data.progress)

            # Status change auto-fill for completed_at
            if "completedAt" in data:
                item.user_data.completed_at = (
                    datetime.fromisoformat(data["completedAt"]).date()
                    if data["completedAt"]
                    else None
                )
            elif new_status != old_status and new_status in [
                "Anticipating",
                "Completed",
            ]:
            ...
            if "episodeCount" in data:
                item.metadata_info.episode_count = data["episodeCount"]
            if "volumeCount" in data:
                item.metadata_info.volume_count = data["volumeCount"]
            if "chapterCount" in data:
                item.metadata_info.chapter_count = data["chapterCount"]
            if "wordCount" in data:
                item.metadata_info.word_count = data["wordCount"]
            if "pageCount" in data:
                item.metadata_info.page_count = data["pageCount"]
            if "avgDurationMinutes" in data:
                item.metadata_info.avg_duration_minutes = data["avgDurationMinutes"]
